  ...
  <>
    ...
    <div className='flex flex-col items-center justify-evenly h-full pl-2 pr-2 w-full'>
      <h1 className='text-text text-xl font-bold text-center self-start w-full'>
        {user_name}'s <br/> {game_name} statistics
      </h1>

      <div className='text-center'>
        <p>Play time: {(statistics.play_time/60).toFixed(0)}h</p>
        { statistics.similar_achievements != null && <p>Similar achievements: {(statistics.similar_achievements * 100).toFixed(0)}%</p> }
        <p className='mb-auto'>Game Completion: {(statistics.game_completion * 100).toFixed(0)}%</p>
      </div>
    </div>
  </>
);